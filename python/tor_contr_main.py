...
directory = os.getcwd() + '/Test_EXP_001_04_05_2023_newcontrol_dist_initial_guess_vid'
...
# q0 = np.array([-0.2, 0.1, 0]) # pcc
# q0 = pcc2delta0(q0, C.d)
q0 = np.array([-0.07, -0.03, 0]) # Delta
...
for i in range(0, C.max_iter):
    ...
    ## offset to tire each tendon which will be a reference at the beginning of the exp. 0.1 is the minimum value you can set to check how much they are loose
    ...
    ## data2save 
    ## This is useless, all data are saved into a cssFile but I already did this for analyze the data 
    ...
    ilc_controller = ILC_SoftRobot(robot,
                                   ...
                                   q_dot_des,
                                   q_ddot_des)
    ## time_span X component
    if i >= 1:
        path_old_control = directory + '/Iter_{}/'.format(i - 1)
        ...
        if C.WANNA_MODEL_BASED_INIT:
            u_old = ilc_controller.initalGuessModelbased()
            u_old = np.vstack((np.zeros((C.max_step, C.n_actuator)), u_old))
            u_old = u_old/0.5
            ## if C.WANNA_PLOT:
            if True:    
                plt.figure(clear=True)
                ...
                for i_a in range(0, C.n_actuator):
                    plt.plot(np.linspace(0, 2 * C.max_step * C.dt, 2 * C.max_step), u_old[:, i_a], color=C.color_list[i_a],\
                            label='u_' + str(i_a + 1), linewidth=3)
                plt.grid()
                plt.legend(loc='best', shadow=True, fontsize=10)
                plt.show()
        else:
            u_old = np.zeros((2 * C.max_step, C.n_actuator))
    ...
    for j in range(0, 2 * C.max_step): ## time
        try:
            ...
            if loadcell.newMsgReceived:
                ...
                # if j >= C.max_step and C.PRINT_ONE_ILC:
                #     print(colored('========================================================================================', 'yellow'))
                #     print(colored('[INFO]:\tThe ILC starts now ...', 'yellow'))
                #     print(colored('========================================================================================', 'yellow'))
                #     C.PRINT_ONE_ILC = False

                dt_exp_vett.append(t - t_old) # ~10 Hz

                ## Note that the ui are coherent with the number of the top of the motor in the real system, while u_old depends on my model

                if C.JUST_TORQUE_CONTR:
                ######################################################################################################
                ######## TORQUE CONTROL TO THE ACTUATOR DIRECTLY
                ######################################################################################################
                    ...
                    u1 = e1 * k_p + u1_pre - u_old[j, 0] * gain_check ## This rotates the opposite way, check the tendon pulley
                    u2 = e2 * k_p + u2_pre + u_old[j, 1] * gain_check 
                    u3 = e3 * k_p + u3_pre + u_old[j, 2] * gain_check
                    u4 = e4 * k_p + u4_pre + u_old[j, 3] * gain_check 
                else:

                ######################################################################################################
                ######## TORUQE CONTROL ON THE LOADCELL
                ######################################################################################################
                    ...
                    ## security check 
                    if abs(u1) > C.u_sat:
                        u1 = np.sign(u1) * C.u_sat
                    
                    if abs(u2) > C.u_sat:
                        u2 = np.sign(u2) * C.u_sat
                    
                    if abs(u3) > C.u_sat:
                        u3 = np.sign(u3) * C.u_sat

                    if abs(u4) > C.u_sat:
                        u4 = np.sign(u4) * C.u_sat
                ...
                q_vett.append(q)
                q_des_vett.append(q_des[conta_time,:]) # not used wrong 
                q_dot_des_vett.append(q_dot_des[conta_time,:]) # not used wrong 
    ...
    print('[INFO]:\tInit displ too big ILC wont work like that :(, i.e., [{}, {}, {}]'\
          .format(np.round(disp_init[0],3), np.round(disp_init[1],3), np.round(disp_init[2],3)))
    print('========================================================================================')
    q_vett = q_vett + disp_init

    # q_vett = removeOutliersMatrix(q_vett)
    # q_vett = lowPassFilter(q_vett) # it makes thing worst
    # q_dot_vett = lowPassFilter(q_dot_vett)
    # q_ddot_vett = lowPassFilter(q_ddot_vett)
    ...
    ## compute the new control action ILC
    ...
    plt.figure(clear=True)
    plt.xlabel(r'Time $[s]$')
    plt.ylabel(r'New Control Input')
    for i_a in range(0, C.n_actuator):
        plt.plot(time_vett, u_new[:, i_a], color=C.color_list[i_a],\
    ...
    u_old = u_new * gain_check ## to test the correctceness of the code

    ## computing error within the iteration
    for iii in range(0, C.n_output):
        MSE[iii] = np.square(error_ilc_pos[:, iii]).mean()
        err_RMS[i] = math.sqrt(np.linalg.norm(MSE, 2))

    print(colored('========================================================================================', 'yellow'))
    print(colored('[INFO]:\t\tIteration {} lasts {} [sec] with dt: {} [sec]'.format(i, np.round(t,2), np.round(dt_exp, 3)),'yellow'))
    print(colored('[INFO]:\t\tErr RMS is {}'.format(np.round(err_RMS[i], 5)), 'yellow'))
    print(colored('========================================================================================', 'yellow'))

    if i > 0:
        if err_RMS[i] > err_RMS[i-1]:
    ...
    if err_RMS[i] <= C.toll_err:
        print(colored('\tLEARNING HAS BEEN ACHIVED ...\n\tNON CI CREDDOOOOOOOOOOO','green'))

        plt.figure(clear=True)
        plt.ylabel(r'RMS')
        plt.xlabel(r'Iterations')
        plt.plot(range(0, i+1), err_RMS[0:i+1], color='red', marker='o', linestyle='-.', linewidth=C.line_widt, markersize=C.marker_size)
        plt.grid()
        plt.savefig(directory + '/' +'rms_final.svg', format='svg')
        plt.show()

        plt.figure(clear=True)
        plt.ylabel(r'Tracking $[N]$')
        plt.xlabel(r'Time $[s]$')
        for ii in range(0, C.n_state): ## C.n_output (same for PCC)
            ## assuming matrix n_time x n_actuators
            plt.plot(time_vett[-C.max_step:], q_des[-C.max_step:, ii], label=C.label_list_q[ii], linewidth=C.line_widt, color=C.color_list[ii])
            plt.plot(time_vett, q_vett[:, ii], label=C.label_list_q[ii], linewidth=C.line_widt, color=C.color_list[ii], linestyle='-.')
        plt.grid()
        plt.legend(loc='best', shadow = True, fontsize=C.font_size)
        plt.savefig(directory + 'plot/' + 'track_final.svg', format='svg')
